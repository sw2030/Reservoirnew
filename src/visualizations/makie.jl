...
cd(@__DIR__)
psps = load("pressure4.jld")["pressure_ver4"]
...
sliders = ntuple(3) do i
    ...
    hmap = contour!(
        ...
        interpolate = true, linewidth = 0.1#, color="RdBu"
    )
...
b2 = button(translated(gui), "3d/2d"; dimensions = (80, 40)) do clicks
    if iseven(clicks)
        cam3d!(hscene)
    else
        cam2d!(hscene)
...
Makie.vbox(sliders..., b, b2)
colorlegend(scene, :viridis, (mini, maxi), widths = (20, 800))
