...
function res_f(m, Δt, g, g_prev, i, j, k;bth=true, p_bth=4000.0, maxinj = 10000.0)
    ...
    # Injection
    q_water = g[4,1]>maxinj ? 0.0 : m.q_water[i,j,k]  ## if pressure is larger than max, no injection
    ...
    Φ_diff_oil   = p_oil_ijk-p_bth-m.ρ_o(p_oil_ijk)*(m.z[i,j,k]-12001)/144
    Φ_diff_water = p_water_ijk-p_bth-m.ρ_w(p_water_ijk)*(m.z[i,j,k]-12001)/144
    well_oil     = PI==0.0 ? 0.0 : PI*Φ_diff_oil*m.k_r_o(S_water_ijk)*m.ρ_o(p_oil_ijk)/m.μ_o
    well_water   = PI==0.0 ? 0.0 : PI*Φ_diff_water*m.k_r_w(S_water_ijk)*m.ρ_w(p_water_ijk)/m.μ_w
    ...
    residual_water_ijk = T_water_west*(Φ_water_im1jk - Φ_water_ijk) +
                       ...
                       q_water*m.ρ_w(p_water_ijk)                    -
                       well_water                                    -
                       (V_ijk*S_water_ijk*m.ρ_w(p_water_ijk)         -
                       V_ijk_prev*S_water_prev*m.ρ_w(p_water_prev))/Δt
    ...
    residual_oil_ijk   = T_oil_west*(Φ_oil_im1jk - Φ_oil_ijk)      +
                       ...
                       V_ijk_prev*S_oil_prev*m.ρ_o(p_oil_prev))/Δt
