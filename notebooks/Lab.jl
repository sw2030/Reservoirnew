...
R = ReservoirSolve(m, [0.005], g_guess; tol_relnorm=0.99, tol_gmres=0.99); ##Compiling
close(R)

forinput(R) = cat(Array(R[1].A), Array(R[2].A);dims = 4);
forload(R) = (makegrid(distribute(R[:,:,:,1]),7), makegrid(distribute(R[:,:,:,2]),7))